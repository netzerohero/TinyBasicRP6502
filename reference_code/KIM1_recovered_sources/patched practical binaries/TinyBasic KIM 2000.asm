;
; Tiny Basic Tom Pittmman
;
; reconstructed version of the KIM-1 versions from tape Hans Oten
; delivers byte for byte equal version of binary 0200- or 2000-
; KIM breaktest included, removed from $0100 to end of program
; - Prompt changed to '>'
; - backspace is $08
; - paddding removed
;
...
; KIM defines 
;
GETCH    = $1E5A          ; KIM character input routine
OUTCH    = $1EA0          ; KIM character output routine
FREERAM  = $2900          ; Start of free RAM for program storage. must be page boundary, $0B00 low version for high version $2900
KTTY	 = $1740		  ; Break test if PB7 low at TTY input  
;
; Tiny Basic starts here
;
         .org     $2000             ; Start of Basic. KIM versions at $0200 and $2000 
START
...
; Some codes
;
BSC      .byte $08                   ; Backspace code was $5f
LSC      .byte $18                   ; Line cancel code
PCC      .byte $00                   ; Pad character control was DEFAULT 84 (pad enabled bit 7, 4 pad characters
TMC      .byte $80                   ; Tape mode control
...
;
COLD_S   lda #FREERAM&$FF          ; low byte for start of RAM
		 sta $20
         sta $22                    ; Store $00 in $22
         lda #FREERAM/256           ; high byte fo start of RAM
...
MEM_T    lda ($22),Y                ; Load accumulator With the contents of a byte of memory
         ...
         bne PULL                 ; Skip if we don't need to increment page
...
;
;
         .byte $02                   ; No idea about this
         .byte $53                   ; No idea about this
;
;
...
LBL005   cmp #$30                   ;
         ...
         asl a                       ; Multiply the OP code by 2 
...
LBL015   jsr P_NWLN                 ; Go print CR, LF and pad characters
         ...
         lda $BE
         beq LBL012
         lda #ERRSTR &$FF           ; Get lo byte of error string address
         sta $2A                    ; Put in $2A
         lda #ERRSTR/256            ; Get hi byte of error string address
         sta $2B                    ; Put in $2B
...
LBL087   lda #$FF                   ; Normal entry point - Set pad to $FF !ho  patch TO 00
...
; TBIL program table
;
ILTBL    .byte $24, $3E, $80, $27, $10, $E1, $59, $C5, $2A, $56, $10, $11, $2C, $8B, $4C
         ...
         .byte $E7, $24, $3F, $20, $80, $27, $E1, $59, $81, $AC, $30, $BC, $13, $11, $82
         ...
         .byte $84, $BD, $09, $06, $2F, $84, $BC, $09, $95, $2F, $09, $04, $2F
         .byte $00, $00, $00
; End of Tiny Basic
...
;  BREAK TEST FOR TIM
; 
;TIMBT    lda	#$01		; if TTY bit 1 then there is incoming character?
;		 bit 	MPB 
;		 clc
;    	 beq	TIMBT1		
;TIMBT2	 bit	MPB 		; wait till TTY bit PB0 is 0 
;		 bne 	TIMBT2
;		 sec
;TIMBT1	 rts		 
